...
network *load_network(char *cfg_w, char *weights_w, int clear)
{
    setlocale(LC_ALL, "");

    printf("cheguei em load_network\n");

    // Calcular o tamanho necessário para os buffers (com margem de segurança)
    size_t cfg_len = wcslen(cfg_w) * MB_CUR_MAX + 1;
    size_t weights_len = wcslen(weights_w) * MB_CUR_MAX + 1;

    // Alocar dinamicamente os buffers
    char *char_cfg = (char *)malloc(cfg_len);
    char *char_weights = (char *)malloc(weights_len);

    if (!char_cfg || !char_weights) {
    ...
    // Converter wchar_t para char
    size_t len_cfg = wcstombs(char_cfg, cfg_w, cfg_len);
    size_t len_weights = wcstombs(char_weights, weights_w, weights_len);

    if (len_cfg == (size_t)-1 || len_weights == (size_t)-1) {
        fprintf(stderr, "Falha na conversão de wide-char para char!\n");
        free(char_cfg);
        free(char_weights);
        return NULL;
    }
    ...
    // Logs detalhados
    printf("char_cfg: '%s'\n", char_cfg);
    printf("char_weights: '%s'\n", char_weights);

    // Verificação de terminadores
    printf("Último caractere char_cfg: '%c'\n", char_cfg[len_cfg - 1]);
    printf("Último caractere char_weights: '%c'\n", char_weights[len_weights - 1]);

    // Validar arquivos
    FILE *test_cfg = fopen(char_cfg, "r");
    if (!test_cfg) {
        fprintf(stderr, "Arquivo cfg não encontrado: %s\n", char_cfg);
        free(char_cfg);
        free(char_weights);
        return NULL;
    }
    ...
    FILE *test_weights = fopen(char_weights, "r");
    if (!test_weights) {
        fprintf(stderr, "Arquivo weights não encontrado: %s\n", char_weights);
        free(char_cfg);
        free(char_weights);
        return NULL;
    }
    ...
    // Carrega a rede
    network *net = parse_network_cfg(char_cfg);
    if (char_weights[0] != '\0') {
        load_weights(net, char_weights);
    }

    if (clear && net && net->seen) {
        *(net->seen) = 0;
    }
