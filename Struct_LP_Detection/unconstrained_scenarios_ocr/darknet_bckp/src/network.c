...
{   //Convert to simple char
    setlocale(LC_ALL, "");
    printf("cheguei em load network\n");
    char char_weights[256];
    char char_cfg[256];

    size_t len1 = wcstombs(char_weights, weights, sizeof(char_weights));
    size_t len2 = wcstombs(char_cfg, cfg, sizeof(char_cfg));

    if (len1 > 0 && len2 > 0) {
        printf("Caracteres convertidos: %s    %s\n", char_weights, char_cfg);
    } else {
        printf("Falha na conversão.\n");
    }

    network *net = parse_network_cfg(char_cfg);
    if(char_weights && char_weights[0] != 0){
