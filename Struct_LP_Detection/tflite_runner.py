...
class YOLOv11TFLite:
    ...
    Attributes:
        model (Interpreter): TensorFlow Lite interpreter for the YOLOv11 model.
        ...
        int11 (bool): Whether the model uses int8 quantization.
    ...
    Examples:
        ...
        >>> detector = YOLOv11TFLite("yolov11n.tflite", conf=0.25, iou=0.45)
    ...
    def __init__(self, model: str, conf: float = 0.25, iou: float = 0.45, metadata: str | None = None):
        """
        Initialize the YOLOv11TFLite detector.
    ...
    def postprocess(self, img: np.ndarray, outputs: np.ndarray, pad: tuple[float, float]) -> np.ndarray:
        """
        Process model outputs to extract and visualize detections. (VERSÃO CORRIGIDA)

        Args:
            img (np.ndarray): The original input image.
            outputs (np.ndarray): Raw model outputs.
            pad (tuple[float, float]): Padding ratios from preprocessing (não será mais usado).
        ...
        # O output do modelo geralmente é (batch, 84, 8400) ou similar. Transpomos para (batch, 8400, 84)
        # para que cada linha represente uma possível detecção.
        outputs = outputs.transpose(0, 2, 1)

        # Converter as coordenadas do centro (cx, cy) para o canto superior esquerdo (x1, y1)
        outputs[..., 0] -= outputs[..., 2] / 2  # x center to top-left x
        outputs[..., 1] -= outputs[..., 3] / 2  # y center to top-left y

        # Calcular os fatores de escala e preenchimento para remapear as coordenadas
        gain = min(self.in_height / h, self.in_width / w)
        pad_x = (self.in_width - w * gain) / 2
        pad_y = (self.in_height - h * gain) / 2
        
        for out in outputs:  # Iterar sobre o batch (normalmente apenas uma imagem)
            # Obter pontuações e aplicar o limiar de confiança
            scores = out[:, 4:].max(-1)
            keep = scores > self.conf
            
            # Manter apenas as detecções que passaram no filtro de confiança
            boxes = out[keep, :4]
            scores = scores[keep]
            class_ids = out[keep, 4:].argmax(-1)
            

            # Remover o preenchimento (padding) e escalar as coordenadas de volta para o tamanho da imagem original
            boxes[:, 0] = (boxes[:, 0] - pad_x) / gain
            boxes[:, 1] = (boxes[:, 1] - pad_y) / gain
            boxes[:, 2] = boxes[:, 2] / gain
            boxes[:, 3] = boxes[:, 3] / gain

            # Aplicar supressão não máxima (NMS)
            # cv2.dnn.NMSBoxes espera as caixas no formato [x, y, w, h] e que sejam do tipo float32
            indices = cv2.dnn.NMSBoxes(boxes.astype(np.float32), scores.astype(np.float32), self.conf, self.iou)
            if indices is not None:
                # O NMS pode retornar um array aninhado, então usamos .flatten()
                indices = indices.flatten()
                # Desenhar as detecções que sobreviveram ao NMS
                #for i in indices:
                #    self.draw_detections(img, boxes[i], scores[i], class_ids[i])

                return sorted([( boxes[i], scores[i] ) 
                    for i in indices 
    ...
    def detect(self, img: np.ndarray) -> np.ndarray:
        ...
        Args:
            img: np.ndarray input image matrix.

        Returns:
            (np.ndarray): The output image with drawn detections.
        """

        img = cv2.resize(img, (self.in_width, self.in_height))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="./models/yolo11n_saved_model/yolo11n_float16.tflite",
        help="Path to TFLite model.",
    )
    parser.add_argument("--img", type=str, default=str("car.jpg"), help="Path to input image")
    parser.add_argument("--conf", type=float, default=0.25, help="Confidence threshold")
    parser.add_argument("--iou", type=float, default=0.45, help="NMS IoU threshold")
    parser.add_argument("--metadata", type=str, default="./models/yolo11n_saved_model/metadata.yaml", help="Metadata yaml")
    args = parser.parse_args()

    detector = YOLOv11TFLite(args.model, args.conf, args.iou, args.metadata)
    img = cv2.imread(str("car.jpg"))
